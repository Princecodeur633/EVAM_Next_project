  ...
  return (
    <div className="min-h-screen bg-bg flex relative">
      <button
        ...
        className="absolute top-4 right-4 z-10 h-9 w-9 flex items-center justify-center border border-line rounded-[8px] text-muted hover:text-ink bg-surface shadow-sm"
      >
        {theme === "dark" ? <Sun size={15} /> : <Moon size={15} />}
      </button>

      <div className="hidden lg:flex w-[38%] bg-sidebar text-white flex-col justify-between p-12 relative overflow-hidden">
        <div className="absolute inset-0 opacity-30 evam-grid-bg pointer-events-none" />
        <div className="relative">
          <p className="text-[11px] uppercase tracking-[0.22em] text-white/45">Unité de production</p>
          <h1 className="text-[44px] font-bold tracking-[0.2em] mt-4">EVAM</h1>
        ...
        <div className="relative text-[13px] text-white/50 space-y-3">
      ...
      <div className="flex-1 overflow-y-auto p-6 lg:p-10">
        <div className="max-w-[720px] mx-auto">
          <p className="text-[11px] uppercase tracking-[0.16em] text-muted font-medium">Connexion</p>
          <h2 className="text-[26px] font-semibold mt-1 tracking-tight">Choisir un poste</h2>
          <p className="text-muted text-[13px] mt-1 mb-6">
            Chaque profil ouvre une interface différente. Cliquez une carte pour entrer — mot de passe démo :{" "}
            <span className="num text-ink font-medium">evam</span>
          </p>
            ...
            {state.users.filter((u) => u.active).map((u) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "text-left rounded-[10px] border p-3.5 transition-all bg-surface",
                    active ? "border-primary bg-primary-soft/50 shadow-[var(--shadow)]" : "border-line hover:border-line-strong",
                  )}
                >
                  <div className="flex items-start gap-3">
                    <span className={cn("h-8 w-8 rounded-[7px] text-white flex items-center justify-center shrink-0", ACCENT_CLASS[p.accent])}>
          ...
          <form onSubmit={onSubmit} className="evam-card p-4 grid sm:grid-cols-[1fr_1fr_auto] gap-3 items-end">
